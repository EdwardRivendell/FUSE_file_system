...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    /* 填充super信息 TODO*/
    int io_size, disk_size;
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &io_size);
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &disk_size);//the return value is the state code of function instead of real result
    super.sz_io = io_size;
    super.sz_disk = disk_size;
    super.sz_blks = 2*io_size; 
